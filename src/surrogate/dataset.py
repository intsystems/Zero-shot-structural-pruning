...
def create_surrogate_dataset_bern(graph_model, test_loader, criterion, n_iterations, mask_prob, device):
    """
    Create a dataset for surrogate model with Bernoulli sampling. Returns train and test Torch datasets.
    """

    graph_model.eval()

    n_edges = len(graph_model.edges)
    n_iterations_train = n_edges + 2  # as default
    masks_train = []
    losses_train = []
    masks_set = set([-1])

    # Sampling masks and calculate loss
    tbar = trange(n_iterations_train, desc='Train masks', leave=False)
    for i in tbar:
        update_log(i, 'create_surrogate_dataset_train')
