\documentclass{beamer}

% \graphicspath{{figs}}  % your figures here

\usepackage[T1,T2A]{fontenc}
\usepackage[main=russian,english]{babel}

\usepackage{cmap}
\usepackage{tikz}
\usepackage{array}
\usepackage{multicol}
\usepackage{booktabs}
\usepackage{csquotes}
\usepackage{amssymb,amsfonts,amsmath}

\usecolortheme{beaver}
\usefonttheme[onlymath]{serif}  % math fonts as in article
\setbeamertemplate{navigation symbols}{}
\setbeamertemplate{footline}[page number]
\setbeamertemplate{blocks}[rounded=true,shadow=true]
\setbeamersize{text margin left=0.5cm,text margin right=0.5cm}
\setbeamerfont{author}{size=\normalsize}
\setbeamerfont{institute}{size=\normalsize}
\setbeamerfont{date}{size=\normalsize}
\setbeamertemplate{enumerate item}{\insertenumlabel)}
\setbeamertemplate{enumerate subitem}{\insertenumlabel)}
\setbeamertemplate{enumerate subsubitem}{\insertenumlabel)}



\title[\hbox to 56mm{Structural pruning}]{Структурное прореживание моделей глубокого обучения на основе суррогатных моделей}

\author{М.\,О.~Иванов\inst{} \\
\tt{\footnotesize ivanov.mo@phystech.edu }}
\institute{\inst{} Москва, Московский физико-технический институт \\
\textbf{Научный руководитель}: к.ф.-м.н. Бахтеев Олег Юрьевич} \date{2026}




\title{Структурное прореживание моделей глубокого обучения на основе суррогатных моделей}
\author{
    Иванов~М.\,О.\\
    Научный руководитель: к.ф.-м.н. Бахтеев~О.\,Ю.
}
\institute{
    Кафедра интеллектуальных систем ФПМИ МФТИ\\
    Специализация: Интеллектуальный анализ данных\\
    Направление: 01.03.02 Прикладная математика и информатика
}
\date{24 июня 2026}

\begin{document}

\begin{frame}
    \thispagestyle{empty}
    \maketitle
\end{frame}

\begin{frame}{Структурное прореживание моделей глубокого обучения}
    
    \begin{block}{Цель} 
      Предложить эффективный метод структурного прореживания нейросетей.
    \end{block}
    
    ~\\
    \begin{block}{Проблема}
      Существующие методы работают послойно. Учёт взаимосвязей между слоями модели является вычислительно сложной задачей.
    \end{block}
    ~\\
    \begin{block}{Метод решения}
      Предлагаемый метод основан на построении суррогатной графовой модели, повторяющей структуру исходной нейросети.
    \end{block}
\end{frame}

\begin{frame}{Схема предлагаемого решения}
    \begin{columns}
        \begin{column}{0.4\textwidth}
            На основе вычислительного графа нейросети строится суррогатная графовая модель, оптимизируемая таким образом, чтобы аппроксимировать функцию ошибки.
            
            \vspace{5pt}
            
            Суррогатная модель является критерием при процессе прореживания.
        \end{column}
        \begin{column}{0.6\textwidth}
            \includegraphics[width=\linewidth]{figs/information_graph.eps}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Задача структурного прореживания}
    Архитектура модели $f$ представляет собой ориентированный ациклический граф $G = (V, E)$. Каждая вершина $v_i \in V$ соответствует слою нейронной сети, и каждое ребро $e_{ij} \in E$ представляет поток данных из выхода слоя $v_i$ до входа $v_j$. 

    \vspace{10pt}
    
    В данной формулировке удаление вершины означает удаление соответствующего слоя из сети, тогда как удаление ребра~--- к устранению зависимости передачи данных между слоями (например, в случае residual connection).

    \vspace{10pt}
    
    Пусть задана выборка $\mathfrak{D} = \mathfrak{D}_\text{train} \cup \mathfrak{D}_\text{val}$. Задана функция ошибки $S$. Пусть $\mathbf{w}$~--- параметры модели до прореживания, а $\mathbf{w}'$~--- параметры модели после. Кроме того, определим маску рёбер $\mathbf{M} \in \mathbb{Z}_2^{|E|}$, показывающую, является ли ребро удалённым. Тогда задача оптимизации структурного прореживания:
        $$\min_{\mathbf{w}' \in \mathbb{R}^k}
    \left|
    S(\mathbf{w}', \mathbf{M} \mid \mathfrak{D}, f)
    -
    S(\mathbf{w}, \mathbf{M} \mid \mathfrak{D}, f)
    \right|.$$
    
\end{frame}

\begin{frame}{Суррогатная графовая модель}
    \small
   Пусть задано $\boldsymbol{\gamma}_{ij} \in [0, 1]$ для каждого ребра $e_{ij} \in E$. Также для графа $G$ задана маска рёбер $\mathbf{M} \in \mathbb{Z}_2^{|E|}$:
        $$ \mathbf{M}_{ij} = 0 \quad \Leftrightarrow \quad \boldsymbol{\gamma}_{ij} = 0.$$

    \vspace{10pt}
    
    \begin{lemma}
    Графовая модель $G(\boldsymbol{\gamma}, \mathbf{M})$ может быть представлена в виде
    $$
    G(\boldsymbol{\gamma}, \mathbf{M})[\mathbf{x}] = \mathbf{x} \cdot \sum_{p \in P} \prod_{(i,j) \in p} \left( \boldsymbol{\gamma}_{ij} \cdot \mathbf{M}_{ij} \right),
    $$
    где $P$~--- множество всех путей в графе.
    \end{lemma}

    \vspace{10pt}

    Добавляя ограничение на веса рёбер, получаем оптимизационную задачу:
    $$
    \begin{aligned}
    \boldsymbol{\gamma}
    =
    \arg\min_{\boldsymbol{\gamma} \in [0,1]^{|E|}}
    \quad
    & \mathsf{E}_{\mathbf{M}}
    \left\|
    G(\boldsymbol{\gamma}, \mathbf{M})
    -
    S(\mathbf{w}, \mathbf{M} \mid \mathfrak{D}, f)
    \right\|_2^2
    \\
    \textrm{s.t.} \quad
    &
    \sum_{j:\ e_{ij} \in E} \boldsymbol{\gamma}_{ij} = 1.
    \end{aligned}
    $$
\end{frame}

\begin{frame}{Теорема о локальной линейной аппроксимации функции ошибки графовой суррогатной моделью}
    \small
    \begin{theorem}[Ivanov, 2026]
    Пусть функция ошибки $S$ нормирована таким образом, что $S \in [0, 1]$. Для любой фиксированной вершины $i \in V$ пусть $E_i = \{(i,j) \in E\}$ обозначает множество исходящих из неё ребер, а $\mathbf{M}_i = \{\mathbf{M}_{ij} \mid (i,j) \in E_i\}$~--- соответствующее подмножество масок, при этом все остальные маски рёбер в графе зафиксированы и равны $1$.

    Тогда функция ошибки $S$ может быть локально аппроксимирована графовой суррогатной моделью следующим образом:
    
    $$S(\mathbf{w}, \mathbf{M}_i \mid \mathfrak{D}, f) = C + \sum_{(i,j) \in E_i} \boldsymbol{\gamma}_{ij} \mathbf{M}_{ij} + o(\|\mathbf{M}_i - \mathbf{1}\|),$$
    
    где $C$~--- константа, а компоненты вектора коэффициентов определяются как:
    
    $$\boldsymbol{\gamma}_{ij} = \frac{\partial S}{\partial \mathbf{M}_{ij}}.$$
    \end{theorem}
\end{frame}

\begin{frame}{Вычислительный эксперимент}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            Рассматривается задача классификации изображений из датасета MNIST.

            \vspace{10pt}

            \includegraphics[width=\linewidth]{figs/graph_example_1.eps}
            
            В эксперименте проводилась оптимизация суррогатных моделей, построенных на основе семплированных графов из нод $\textbf{ReLU}(\textbf{Linear}(\mathbf{x}))$.  
        \end{column}
        \begin{column}{0.5\textwidth}
            В сравнении участвуют следующие базовые методы:\\
                \vspace{5pt}
                \begin{small}
                \textbf{(Graph)}: предложенный метод,\\
                \vspace{5pt}
                \textbf{(FCN)}: простая нейросеть,\\
                \vspace{5pt}
                \textbf{(Linear)}: линейная регрессия,\\
                \vspace{5pt}
                \textbf{(L1)}: метод, основанный на измерении величин параметров,\\
                \vspace{5pt}
                \textbf{(Taylor)}: метод, использующий приближение Тейлора до 1-го и 2-го порядков.
                \end{small}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Процент пересечения топ-5\% моделей}
    Приводится процент пересечения топ-5\% моделей, ранжированных в соответствии с истинной функцией ошибки, и моделей, ранжированных в соответствии с прогнозом суррогатной модели.
  \begin{table}[H]
    \centering
    \begin{tabular}{|r|c|c|c|c|}
    \hline
    \bfseries Model & 10--15 edges & 20--40 edges & 50--100 edges & > 100 edges \\
    \hline
    Graph  & \textbf{62.0\%} & \textbf{64.5\%} & \textbf{69.3\%} & \textbf{58.3\%} \\
    FCN & 2\% & 15.2\% & 5.2\% & 6.8\% \\
    Linear & 18.8\% & 9.6\% & 27.2\% & 15.2\% \\
    L1 & 0.0\% & 1.6\% & 6.8\% & 6.8\% \\
    Taylor & 8.0\% & 10.8\% & 29.6\% & 24.4\% \\
    \hline
    \end{tabular}
    \end{table}
  Из таблицы видно, что предложенный метод демонстрирует высокую эффективность при решении предложенной задачи. 
\end{frame}

\begin{frame}{Прогноз функции ошибки против истинных значений}
    \begin{figure}
    \includegraphics[width=0.9\textwidth]{figs/graph_1_iter_2_experiment_6.eps}
    
    \centering
    \includegraphics[width=0.6\textwidth]{figs/graph_2_iter_2_experiment_6.eps}

    \vspace{10pt}
    
    График истинных и прогнозируемых значений ошибок для сравнения методов. Используется модель с 10--15 рёбрами.
    \label{graph_iter_2}
    \end{figure}
\end{frame}

\begin{frame}{Выносится на защиту}
    \small
    \begin{block}{Достигнутые результаты}
    \begin{enumerate}
      \item Доказана теорема о локальном поведении графовой суррогатной модели.
      \item Проведены эксперименты, показавшие, что предложенный метод превосходит суррогатные модели, которые не используют информацию о вычислительном графе.
      \item Планируется расширить и обобщить этот метод для прореживания по каналам (например, по фильтрам в свёрточных нейросетях).
    \end{enumerate}
    \end{block}
    
    \begin{block}{Доклады по теме работы}
    \begin{enumerate}
        \item Работа представлена на Всероссийской научной конференции МФТИ 2026.
        \item Статья подготовлена к публикации, планируется подача в рецензируемый журнал.
    \end{enumerate}
    \end{block}
\end{frame}

\end{document} 